...
import { type SolutionStage, type WpId } from "@/lib/constants";
      ...
      fontFamily: FONT_STACK
            ...
            return <button key={stage.key} type="button" onClick={() => setActiveStage(prev => prev === stage.key ? null : stage.key)} style={{
              ...
              fontFamily: FONT_STACK
                  ...
                  <span className="[font-size:13px] [color:#666666]">
                    {count} {count === 1 ? t.solutions.responseOne : t.solutions.responseOther}
                  </span>
                </button>;
          })}
          </div>
        ...
        <section>
          <h2 className="[font-size:26px] [font-weight:700] [color:#2a2859] [margin-bottom:24px] [letter-spacing:-0.01em]">
            {activeStage
              ? fill(t.solutions.stageResponses, {
                  stage: tax.stages.find(s => s.key === activeStage)?.label ?? activeStage,
                })
              : t.solutions.allResponses}
          </h2>

          {filtered.length === 0 ? <p className="[color:#808080]">
              {t.solutions.emptyStage}
    ...
    <section className="[margin-top:96px] [padding-top:48px] [border-top:1px_solid_#e6e6e6]">
      ...
      {monthOptions.length > 0 && (
        <div className="[display:flex] [align-items:center] [gap:12px] [flex-wrap:wrap] [margin-bottom:32px]">
          ...
          >
            <option value="all">{t.solutions.allMonths}</option>
            {monthOptions.map((m) => (
              <option key={m} value={m}>
                {formatMonthLong(m, intlLocale)}
              </option>
            ))}
          </select>
          {monthFilter !== "all" && (
      ...
      {!hasAnyPublished ? (
        <p className="[font-size:15px] [line-height:1.7] [color:#808080] [font-style:italic]">
          {t.solutions.noReportsYet}
        </p>
      ) : visibleWps.length === 0 ? (
        <p className="[font-size:15px] [line-height:1.7] [color:#808080] [font-style:italic]">
          {fill(t.solutions.noReportsForMonth, { month: formatMonthLong(monthFilter, intlLocale) })}
        </p>
      ) : (
    ...
    <article className="[background:#ffffff] [border:1px_solid_#e6e6e6] [border-radius:8px] [padding:24px] [display:flex] [flex-direction:column] [gap:16px]">
      ...
      {!latest ? (
        <p className="[font-size:13px] [color:#9a9a9a] [font-style:italic]">
          {t.solutions.noPublishedReports}
        </p>
      ) : (
        <>
          ...
          {earlier.length > 0 && (
            <div className="[margin-top:8px] [border-top:1px_solid_#f2f2f2] [padding-top:16px]">
              ...
              >
                {archiveOpen
                  ? t.solutions.hideEarlier
                  : fill(t.solutions.earlierMonths, { n: earlier.length })}
              </button>
              {archiveOpen && (
...
function FeaturedReport({ report }: { report: WpReport }) {
  ...
  return (
    <div className="[display:flex] [flex-direction:column] [gap:12px]">
      <p className="[font-size:15px] [font-weight:600] [color:#2a2859]">{formatMonthLong(report.month, intlLocale)}</p>
      {report.summary && (
      ...
      {report.next_steps && (
        <p className="[font-size:12px] [color:#666666] [line-height:1.6] [padding-left:12px] [border-left:2px_solid_#e6e6e6]">
          <span className="[font-weight:600] [color:#2a2859]">{t.solutions.nextSteps}</span> {report.next_steps}
        </p>
      )}
      <p className="[font-size:11px] [color:#9a9a9a]">
        {report.interviewer}
        {report.interviewee && <> · {t.solutions.interviewWith} {report.interviewee}</>}
      </p>
    </div>
  );
}
...
function ArchiveReport({ report }: { report: WpReport }) {
  ...
  return (
    <div className="[padding:12px] [background:#f9f9f9] [border:1px_solid_#f2f2f2] [border-radius:6px] [display:flex] [flex-direction:column] [gap:6px]">
      <p className="[font-size:12px] [font-weight:600] [color:#2a2859]">{formatMonthShort(report.month, intlLocale)}</p>
      {report.summary && (
...
function formatMonthLong(isoDate: string, locale: string): string {
  const d = new Date(isoDate);
  const formatted = d.toLocaleDateString(locale, { month: "long", year: "numeric" });
  return formatted.charAt(0).toUpperCase() + formatted.slice(1);
}

function formatMonthShort(isoDate: string, locale: string): string {
  const d = new Date(isoDate);
...
function SolutionCard({
  ...
  const stage = tax.stages.find(s => s.key === solution.stage);
  return <article className="[background:#ffffff] [border:1px_solid_#e6e6e6] [border-radius:8px] [padding:24px] [display:flex] [flex-direction:column] [gap:16px]">
      ...
      {sourceStories.length > 0 && <div className="[margin-top:auto] [padding-top:16px] [border-top:1px_solid_#e6e6e6]">
          ...
          {sourceStories.map(s => <Link key={s.id} href={href(`/story/${s.id}`)} className="[display:block] [font-size:14px] [color:#1f42aa] [text-decoration:none] [padding:4px_0] [font-weight:500]">
              {s.title} →
            </Link>)}
        </div>}
    </article>;

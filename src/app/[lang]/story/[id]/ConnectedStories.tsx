...
function buildGroups(
  ...
  const addToGroup = (kind: Group["kind"], key: string, other: PublicStory) => {
    ...
    const meta =
      kind === "friction"
        ? tax.frictions[key as CareFriction]
        : tax.qualities[key as CareQuality];
...
export default function ConnectedStories({
  ...
  const groups = useMemo(
    () => buildGroups(story, allStories, connections, tax),
    [story, allStories, connections, tax],
  );
    ...
    <div style={{ borderLeft: `3px solid ${group.color}`, paddingLeft: 16 }}>
      <div className="[display:flex] [align-items:baseline] [gap:10px] [margin-bottom:12px]">
        ...
        <span className="[font-size:12px] [color:#9a9a9a]">
          {group.stories.length} {group.stories.length === 1 ? t.common.storyOne : t.common.storyOther}
        </span>
      </div>
      ...
      {hiddenCount > 0 && (
        ...
        >
          {expanded ? t.story.showFewer : fill(t.story.showAllN, { n: group.stories.length })}
        </button>
      )}
    </div>
  );

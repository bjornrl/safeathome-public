...
export async function generateMetadata({
  params
}: PageProps<"/[lang]/story/[id]">): Promise<Metadata> {
...
export default async function StoryPage({
  ...
  return <>
      ...
      fontFamily: FONT_STACK
        ...
        <Link href={href("/internal/content?tab=frictions")} className="[display:inline-flex] [align-items:center] [gap:6px] [font-size:13px] [color:#1f42aa] [text-decoration:none] [margin-bottom:32px] [font-weight:500]">
          {t.story.backToFrictions}
        </Link>
        ...
        <p className="[font-size:12px] [font-weight:600] [text-transform:uppercase] [letter-spacing:0.14em] [color:#808080] [margin-bottom:16px]">
          {tax.scales[story.map_scale]?.label ?? story.map_scale}
        ...
        <div className="[display:flex] [flex-wrap:wrap] [gap:6px] [margin-bottom:40px]">
          {story.frictions?.map(f => <Link key={f} href={href(`/internal/content?tab=frictions&friction=${f}`)} style={{
          ...
          {story.qualities?.map(q => <Link key={q} href={href("/internal/content?tab=qualities")} style={{
          background: tax.qualities[q]?.color + "18",
          color: tax.qualities[q]?.color
        }} className="[font-size:11px] [padding:3px_10px] [border-radius:4px] [font-weight:500] [text-decoration:none]">
                    ...
                    {(r.frictions.length > 0 || r.qualities.length > 0) && (
                      <div className="[display:flex] [flex-wrap:wrap] [gap:4px]">
                        {r.frictions.map(f => {
                          ...
                          return <Link key={f} href={href(`/internal/content?tab=frictions&friction=${f}`)} style={{
                        ...
                        {r.qualities.map(q => {
                          ...
                          return <Link key={q} href={href(`/internal/content?tab=qualities#${q}`)} style={{
                            background: emphasized ? color : color + "18",
                            color: emphasized ? "#ffffff" : color,
                            fontWeight: emphasized ? 600 : 500,
                          }} className="[font-size:11px] [padding:3px_10px] [border-radius:4px] [text-decoration:none]">

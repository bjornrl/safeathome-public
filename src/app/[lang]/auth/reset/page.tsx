...
export default function ResetPasswordPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setSubmitting(false);
    if (updateErr) {
      setError(authErrorMessage(updateErr, "password", t.auth));
      return;
    }
    setNotice(t.reset.saved);
    ...
    <>
      ...
      >
        ...
        {hasRecovery !== false && (
          <form
            onSubmit={onSubmit}
            className="[display:flex] [flex-direction:column] [gap:16px] [padding:24px] [background:#ffffff] [border:1px_solid_#e6e6e6] [border-radius:8px]"
          >
            ...
            {error && (
              <p className="[font-size:13px] [color:#a83f34] [background:#fff2f1] [border:1px_solid_#ffdfdc] [padding:8px_16px] [border-radius:4px]">
                {error}
              </p>
            )}

            {notice && (
              <p className="[font-size:13px] [color:#034b45] [background:#c7fde9] [border:1px_solid_#43f8b6] [padding:8px_16px] [border-radius:4px]">
                {notice}
              </p>
            )}
            ...
            >
              {submitting ? t.reset.submitBusy : t.reset.submit}
            </button>
          </form>
        )}

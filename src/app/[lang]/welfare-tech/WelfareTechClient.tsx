...
function uniqCategories(items: WelfareTechnology[], locale: string): string[] {
...
export default function WelfareTechClient({ items }: { items: WelfareTechnology[] }) {
  ...
  const categories = useMemo(() => uniqCategories(items, intlLocale), [items, intlLocale]);
    ...
    >
      ...
      >
        <header style={{ marginBottom: space.s32 }}>
          ...
          {isAdmin && (
            <Link
              href={href("/admin?tab=welfare-tech")}
              style={{
        ...
        <nav
          role="tablist"
          aria-label={t.welfareTech.filterLabel}
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            {item.category?.trim() || t.welfareTech.uncategorised}
          </span>
          <button
            type="button"
            onClick={onClose}
        ...
        >
          <div style={{ display: "flex", gap: space.s8 }}>
            <ArrowButton onClick={onPrev} disabled={!hasPrev} label={t.common.previous}>
              ←
            </ArrowButton>
            <ArrowButton onClick={onNext} disabled={!hasNext} label={t.common.next}>
              →
            </ArrowButton>
          </div>
          <span
          ...
          >
            {index + 1} {t.common.ofTotal} {total}
          </span>
        </footer>
      </motion.aside>
    </>

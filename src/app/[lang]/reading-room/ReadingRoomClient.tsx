  ...
  return (
    <div style={{ fontFamily: FONT_STACK }}>
      <section style={{ marginBottom: 32, padding: 20, background: "#f9f9f9", border: "1px solid #e6e6e6", borderRadius: 8 }}>
        ...
        <p style={{ fontSize: 11, fontWeight: 600, color: "#666666", marginBottom: 8 }}>{t.readingRoom.frictionsLabel}</p>
        ...
        <p style={{ fontSize: 11, fontWeight: 600, color: "#666666", marginBottom: 8 }}>{t.readingRoom.qualitiesLabel}</p>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          {QUALITY_KEYS.map((k) => {
            const active = selectedQualities.includes(k);
            const color = tax.qualities[k].color;
            return (
              <button
                key={k}
                type="button"
      ...
      {filtered.length === 0 && activeFilters > 0 ? (
        ...
        <ResourceList
          resources={filtered}
          emptyMessage={t.readingRoom.empty}
